...
## Overview
Implement escalation handler service that detects all escalation triggers and generates user-friendly escalation messages. Handles 11 different trigger types and provides appropriate next-step guidance to users.

## Prerequisites
- Task 1 completed (project structure, config, logging)
- Task 8 completed (Intent Classifier)
- Task 9 completed (Router)
- Task 12 completed (Confidence Scorer)
- Virtual environment activated

## Deliverables

### 1. Escalation Handler Service (services/escalation_handler.py)

Create `services/escalation_handler.py` with escalation detection and message generation.

**Key Requirements**:
- All 11 escalation triggers detected
- User-friendly escalation messages
- Mode-specific messages (Customer vs Banker)
- Next-step guidance
- Structured logging

## Escalation Triggers

The escalation handler must detect and handle all 11 trigger types:
...
### Step 1: Escalation Message Templates

```python
# services/escalation_handler.py
...
Document the escalation reason: {reason}""",
    ...
    "low_confidence": """The confidence score for this response is below threshold ({confidence_score} < {threshold}).
...
### Step 2: Escalation Handler Implementation
...
# Escalation messages (from Step 1)
ESCALATION_MESSAGES_CUSTOMER = {...}  # See Step 1
ESCALATION_MESSAGES_BANKER = {...}    # See Step 1
...
class EscalationHandler:
    ...
    async def handle_escalation_async(
        ...
        if trigger_type not in valid_triggers:
            logger.warn("invalid_trigger_type", trigger_type=trigger_type)
    ...
    def _generate_escalation_message(
        ...
        try:
            ...
            logger.warn("message_formatting_failed", trigger_type=trigger_type)
    ...
    def detect_escalation_triggers(
        ...
        if retrieved_chunks and len(retrieved_chunks) > 1:
            ...
            triggers.append("conflicting_evidence")  # Simplified
...
The escalation handler is used throughout the pipeline:
...
if not confidence_result["meets_threshold"]:
    ...
    escalation_result = await escalation_handler.handle_escalation_async(
        ...
        escalation_reason=f"Confidence score {confidence_result['confidence_score']:.2f} below threshold"
    )
    return escalation_result

# After retrieval (if no results)
...
The escalation handler returns:

```python
{
    "escalated": True,
    "escalation_message": "I understand you're asking about account fees. This requires personalized assistance...",
    "trigger_type": "low_confidence",
...
- [ ] Handles invalid trigger types gracefully
...
async def test():
    ...
    print(f"Message: {result['escalation_message']}")
...
for trigger in triggers:
    ...
    assert result["trigger_type"] == trigger
    assert len(result["escalation_message"]) > 0
```
...
triggers = handler.detect_escalation_triggers(
    user_query="I need to speak to a human about my account",
...
assert "insufficient_evidence" in triggers
assert "explicit_human_request" in triggers
```

## Integration Points

- **Task 8 (Intent Classifier)**: Receives intent_category for human_only detection
- **Task 9 (Router)**: Routes to escalation handler for human_only
- **Task 10 (Retrieval Service)**: Detects insufficient_evidence
- **Task 12 (Confidence Scorer)**: Receives confidence_score for low_confidence detection
- **Task 14 (Logging)**: Logs escalation events
- **Task 18 (Main App)**: Used throughout pipeline for escalation
...
Step 1: Intent Classification
    ↓
    if intent_category == "human_only":
        → Escalation Handler (trigger: human_only)
    
Step 3: Retrieval
    ↓
    if no results:
        → Escalation Handler (trigger: insufficient_evidence)
    
Step 5: Confidence Scoring
    ↓
    if confidence < threshold:
        → Escalation Handler (trigger: low_confidence)
...
- **Mode-Specific**: Different messages for Customer vs Banker
- **Next Steps**: All messages include clear next-step guidance
- **Multiple Triggers**: Can detect multiple triggers, uses primary trigger for message
- **Safe Defaults**: Defaults to human_only if trigger type invalid

## Reference

- **DETAILED_PLAN.md** Section 7.6 (Escalation Handler)
- **TASK_BREAKDOWN.md** Task 13
- **Task 8 Guide**: For intent classification output
- **Task 12 Guide**: For confidence scoring output
- **Task 14 Guide**: For logging escalation events
